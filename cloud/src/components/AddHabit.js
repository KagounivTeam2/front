...
const AddHabit = () => {
  ...
  const [recommendations, setRecommendations] = useState([]); // 추천 습관 상태 추가
  ...
  const fetchRecommendations = async () => {
    try {
      const response = await baseAxios.get("/api/habit/recommends"); // 추천 습관 API 호출
      if (response.data.success) {
        setRecommendations(response.data.responseDto.recommentList); // 추천 습관 목록 저장
      } else {
        setError("추천 습관을 가져오는 중 오류가 발생했습니다.");
      }
    } catch (error) {
      console.error("추천 습관을 가져오는 중 오류가 발생했습니다.", error);
      setError("추천 습관을 가져오는 중 오류가 발생했습니다.");
    }
  };

  useEffect(() => {
    fetchRecommendations(); // 컴포넌트 마운트 시 추천 습관 가져오기
  }, []);
      ...
      <div className="add-habit-wrapper">
        <header className="add-habit-header">
          <img
            ...
            onClick={() => navigate("/my-habit")}
          />
          <h1 className="headline">습관 추가</h1>
          <img
            src="img/icon/mypage_logo_x3.png"
            alt="마이페이지 아이콘"
        ...
        <div className="favorite-habit-box">
          {recommendations.length !== 0 &&
            recommendations.slice(0, 3).map((habit, index) => (
              <div
                key={index}
                className="habit-box"
        ...
        <div className="daily-habit-container">
          {[
            {
              text: "매일 오전 9시 전 기상하기",
              image: "/img/recommend_img1.png",
              gradient:
                "linear-gradient(to bottom, rgba(121, 204, 255, 0.7), rgba(213, 252, 255, 0.7))",
            },
            {
              text: "매일 30분 운동하기",
              image: "/img/recommend_img2.png",
              gradient:
                "linear-gradient(to bottom, rgba(254, 160, 184, 0.7), rgba(254, 232, 212, 0.7))",
            },
            {
              text: "매일 물 1L 마시기",
              image: "/img/recommend_img3.png",
              gradient:
                "linear-gradient(to bottom, rgba(99, 95, 184, 0.7), rgba(226, 218, 199, 0.7))",
            },
          ].map((habit, index) => (
            <div
              key={index}
        ...
        <div className="favorite-habit-box">
          {recommendations.slice(3, 6).map((habit, index) => (
            <div
              key={index}
